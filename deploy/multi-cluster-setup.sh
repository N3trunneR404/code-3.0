...
create_cluster() {
    ...
    # Add network optimizations to prevent NIC-related timeouts
    local create_args=(
        "$name"
        --servers "$servers"
        --agents "$agents"
        ...
        --k3s-arg "--kubelet-arg=--node-status-update-frequency=10s@server:*"
        --k3s-arg "--kubelet-arg=--node-status-update-frequency=10s@agent:*"
        --k3s-arg "--kubelet-arg=--runtime-request-timeout=2m@server:*"
        --k3s-arg "--kubelet-arg=--runtime-request-timeout=2m@agent:*"
...
# Add delays between clusters to avoid network interface saturation
...
for spec in "${CLUSTER_SPECS[@]}"; do
    ...
    # Add delay between clusters to avoid network congestion
    # First cluster: no delay, subsequent: 5-10 second delay
    if [ $CLUSTER_COUNT -gt 1 ]; then
        echo "  Waiting 8 seconds to avoid network interface saturation..."
        sleep 8
    fi
    
    create_cluster "$cluster_name" "$cluster_type" "$servers" "$agents" "$cluster_id" "$network_cidr" "$pod_cidr" "$service_cidr" "$accelerator_hint"
    
    # Brief pause after cluster creation to let network settle
    if [ $CLUSTER_COUNT -lt ${#CLUSTER_SPECS[@]} ]; then
        sleep 3
    fi
done
