...
class VirtualTopology:
        ...
        def clone_preserving_clusters(self, *, allowed_nodes: Optional[set[str]] = None) -> "VirtualTopology":
                """Return a fresh allocator that preserves cluster/identity assignments."""

                clone = VirtualTopology()
                clone._cluster_id_seq = self._cluster_id_seq
                clone._cluster_ids = dict(self._cluster_ids)
                clone._cluster_networks = copy.deepcopy(self._cluster_networks)
                clone._node_counters = defaultdict(int, self._node_counters)

                if allowed_nodes is None:
                        allowed_nodes = set(self._identities.keys())
                else:
                        allowed_nodes = set(allowed_nodes)

                for name, identity in self._identities.items():
                        if name in allowed_nodes:
                                clone._identities[name] = copy.deepcopy(identity)
        ...
        # ------------------------------------------------------------------
        def register_cluster(self, cluster: ClusterInfo) -> Tuple[int, bool]:
                """Ensure a cluster has a stable 8-bit identifier and subnets.

                Returns (cluster_id, network_changed) where the boolean indicates
                whether an existing cluster's network profile was updated.
                """
                ...
                prev_network = self._cluster_networks.get(cluster.name)
                profile_changed = False
                if prev_network:
                        network = dict(prev_network)
                else:
                ...
                for key, value in overrides.items():
                        if value and value != network[key]:
                                network[key] = value
                                profile_changed = True

                if prev_network is None or profile_changed:
                        self._cluster_networks[cluster.name] = network

                if prev_network is not None and profile_changed:
                        self._reseed_cluster_network(cluster_id, cluster.name, network)

                cluster.cluster_id = cluster_id
                cluster.network_cidr = network["network_cidr"]
                cluster.pod_cidr = network["pod_cidr"]
                cluster.service_cidr = network["service_cidr"]
                cluster.node_pools = cluster.node_pools or {}

                return cluster_id, bool(prev_network is not None and profile_changed)

        def _reseed_cluster_network(self, cluster_id: int, cluster_name: str, network: Dict[str, str]) -> None:
                """Update node identities to match a new network profile."""
                ...
                for node_name, identity in self._identities.items():
                        ...
                        identity.network_cidr = network["network_cidr"]
                        identity.pod_cidr = network["pod_cidr"]
                        identity.service_cidr = network["service_cidr"]
                        identity.node_ip = self._allocate_ip(network["network_cidr"], identity.node_id)
                        identity.pod_ip = self._allocate_ip(network["pod_cidr"], identity.node_id)
        ...
        def ensure_node(self, cluster_name: str, node: Node) -> Tuple[Optional[NetworkIdentity], Optional[NodePool]]:
                ...
                identity = self._identities.get(node.name)
                if not identity or identity.cluster_id != cluster_id:
...
class DTState:
    ...
    def _compute_and_cache_capacities(self, node: Dict[str, Any]):
        ...
        cpu_legacy = node.get("cpu", {}) or {}
        mem_legacy = node.get("memory", {}) or {}
        gpu_legacy = node.get("gpu", {}) or {}
        k8s = node.get("k8s", {}) or {}

        cores = safe_float(hardware.get("cpu_cores"), 0.0)
        if cores <= 0:
            cores = safe_float(cpu_legacy.get("cores"), 0.0)

        base_ghz = safe_float(hardware.get("base_ghz"), 0.0)
        if base_ghz <= 0:
            base_ghz = safe_float(cpu_legacy.get("base_ghz"), 0.0)

        ram_gb = safe_float(hardware.get("memory_gb"), 0.0)
        if ram_gb <= 0:
            ram_gb = safe_float(mem_legacy.get("ram_gb"), 0.0)

        vram_gb = safe_float(hardware.get("gpu_vram_gb"), 0.0)
        if vram_gb <= 0:
            vram_gb = safe_float(gpu_legacy.get("vram_gb"), 0.0)

        allocatable_cores = safe_float(k8s.get("allocatable_cpu"), cores)
        if allocatable_cores > 0:
            cores = allocatable_cores

        allocatable_mem_gb = safe_float(k8s.get("allocatable_mem_gb"), ram_gb)
        if allocatable_mem_gb > 0:
            ram_gb = allocatable_mem_gb

        # naive "capacity units"
        cpu_units = max(0.0, cores) * max(0.0, base_ghz)
        node["caps"] = {
            "cpu_units": cpu_units,
            "max_cpu_cores": cores,
            "ram_gb": ram_gb,
    ...
    def _apply_virtualization_locked(self, name: str, node: Dict[str, Any]) -> None:
        ...
        _, network_changed = self._virtual.register_cluster(cluster_info)
        if network_changed:
            self._rewrite_cluster_network_descriptors_locked(cluster_name)
    ...
    def _refresh_virtual_topology_from_nodes_locked(self) -> None:
        existing_nodes = set(self.nodes_by_name.keys())
        if isinstance(getattr(self, "_virtual", None), VirtualTopology):
            self._virtual = self._virtual.clone_preserving_clusters(allowed_nodes=existing_nodes)
        else:
            self._virtual = VirtualTopology()
    ...
    def _rewrite_cluster_network_descriptors_locked(self, cluster_name: str) -> None:
        cluster = self.clusters.get(cluster_name)
        if not cluster or not isinstance(getattr(self, "_virtual", None), VirtualTopology):
            return

        for node_name, node_dict in self.nodes_by_name.items():
            labels = ((node_dict.get("k8s") or {}).get("labels") or {})
            labeled_cluster = labels.get("dt.cluster.name")
            if labeled_cluster != cluster_name and node_name not in cluster.nodes:
                continue

            identity = self._virtual.get_identity(node_name)
            if not identity or identity.cluster_name != cluster_name:
                continue

            node_dict["network"] = asdict(identity)
            k8s = node_dict.setdefault("k8s", {})
            k8s_labels = k8s.setdefault("labels", {})
            k8s_labels.setdefault("dt.cluster.name", cluster_name)
            k8s_labels["dt.virtual.cluster_id"] = str(identity.cluster_id)
            k8s_labels["dt.virtual.node_id"] = str(identity.node_id)
            k8s_labels["dt.virtual.mac"] = identity.mac_address
    ...
    def register_cluster(self, cluster_info: ClusterInfo) -> None:
        ...
        with self._lock:
            ...
            if network_changed:
                self._rewrite_cluster_network_descriptors_locked(cluster_info.name)
    ...
    def upsert_node(self, node: Node) -> None:
        ...
        if node.k8s:
            ...
            if cluster_info and node.name not in cluster_info.nodes:
    ...
    def links_incident_to(self, node_name: str) -> List[Link]:
        """Return effective link views touching the given node.

        This mirrors the pre-refactor helper used by the planners and simulator
        and exposes `Link` dataclasses with effective latency/bandwidth values
        derived from the cached link map.
        """

        with self._lock:
            out: List[Link] = []
            for link in self.links_by_key.values():
                a = link.get("a")
                b = link.get("b")
                ...
                out.append(
                    Link(
                        a=str(a),
                        b=str(b),
                        base_latency_ms=float(eff.get("rtt_ms", 0.0)),
                        bandwidth_gbps=float(eff.get("speed_gbps", 0.0)),
                        loss_pct=float(eff.get("loss_pct", 0.0)),
                    )
                )
            return out
